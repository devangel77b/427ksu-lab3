...
%figure 4 please
\begin{table}
\caption{One of the tables created by FizziQ, based on a video of the first trial with selected values to be shown, such as the vertical position (height) and the velocity \cite{fizziq}. The maximum height, velocity upon release, and velocity right after hitting the maximum height are marked with a star. The cells without values are intentionally left blank.}
\label{tab:4}
\begin{center}
\begin{ruledtabular}
\begin{tabular}{cc}
